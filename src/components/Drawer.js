...
export default function Drawer({ open, onClose, side = 'left', children, className }) {
  ...
  return (
    <div className={` overflow-x-hidden overflow-y-auto  inset-0 z-40 target:hidden ${open ? 'fixed' : 'hidden'} `}>
      <div className="opacity-80 fixed inset-0 bg-black" onClick={onClose} />
      <div className={` absolute transition-all z-50  ${classes} ${className}`} >
        {children}
      </div>
    </div>
  )
